...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
    if (solid)
    {
        ...
        {   // map collision
            ...
            if (vellen > 0.1f && this.getMass() > 1.0f &&
		...
		if (blob.getTeamNum() != this.getTeamNum())
        {
			...
			if (mass < veryHeavy && damagePlayer !is null && damagePlayer.getBlob() !is null && damagePlayer.getBlob().getTeamNum() == blob.getTeamNum())
				return;

						
			// hack:for boats killing ppl on top
			if (mass > veryHeavy && blob.getPosition().y < this.getPosition().y && blob.hasTag("flesh")) {
				return;
			}

            // check if we had greater velocity
            if (vellen >= blob.getShape().vellen && vellen > 0.1f && blob.getMass() > 0.0f)
            {
				//printf("? " + blob.getName() + " " +blob.getHealth() + " vellen " + vellen + " " + blob.getShape().vellen);
                hitvel /= vellen;
                hitvec.Normalize();
                coef = hitvec * hitvel;
                coef *= this.get_f32("hit dmg modifier");
                f32 mass = Maths::Min( this.getMass(), 200.0f );
				f32 mass2 = Maths::Min( blob.getMass(), 400.0f );
                f32 dmg = vellen * coef * (mass / mass2) / 8.0f;

				if(this.getName() == "wasp" && (blob.getName() == "stone_door" || blob.getName() == "wooden_door")){
					dmg *= 0.5;
				}
...
void onHitBlob( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData )
{
    ...
    {
		Vec2f force = velocity * this.getMass();
