...
void onTick( CMovement@ this )
{
	...
	if(!(blob.hasTag("fullgravity")) || blob.getVelocity().y > 0){
		return;
	}
	...
	const bool isknocked = blob.exists("knocked") && blob.get_u8("knocked") > 0;
			...
			!onground )											
		{
			...
			if(moveVars.jumpCount > 5) //wait some time to be properly in the air
			{
							...
							!surface_below && !jumpedLEFT && !jumpedRIGHT)
				{
					...
					vel.Set( surface_right ? -walljumpforce : walljumpforce,
							 blob.isKeyPressed(key_action2) ? -1.0f : -2.0f );
					blob.setVelocity(vel);
