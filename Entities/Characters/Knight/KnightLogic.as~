...
void onInit( CBlob@ this )
{
    ...
    this.SetScoreboardVars("ScoreboardIcons.png", 3, Vec2f(16,16));
...
void onTick( CBlob@ this )
{
			...
			( pressed_a2 || (specialShieldState) ) )
    {
        ...
        else
        {
            ...
            {
                if (direction == -1 && !getMap().isInWater( pos + Vec2f(0,16) ) && !moveVars.wallsliding)
                {
            ...
            if (knight.state == KnightStates::shieldgliding && !this.isInWater())
            {
    ...
    else if ( (pressed_a1 || swordState) && !moveVars.wallsliding ) //no attacking during a slide
    {
				...
				knight.state <= KnightStates::sword_cut_down) // cut state
        {
            ...
            if (delta > DELTA_BEGIN_ATTACK && delta < 5)
            {
                f32 attackarc = 90.0f;
        ...
        else if (knight.state == KnightStates::sword_power || knight.state == KnightStates::sword_power_super)
        {
            ...
            {
				knight.swordTimer = 0;
                knight.state = knight.doubleslash ? KnightStates::sword_power : KnightStates::sword_drawn;
		...
		//	if (delta > KnightVars::slash_time / 2 )//&& knight_hit_actor_count(this) > 0)
			{
				//double 
              ...
              delta < KnightVars::slash_move_time )
        {
            ...
            {
                Vec2f slash_vel =  knight.slash_direction * this.getMass() * 0.7f;
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info )
{
	...
	f32 attack_distance = DEFAULT_ATTACK_DISTANCE;

	// add distance
	attack_distance = Maths::Min( attack_distance + Maths::Max( 0.0f, 1.75f*this.getShape().vellen * (vel * thinghy) ), 22.0f );
