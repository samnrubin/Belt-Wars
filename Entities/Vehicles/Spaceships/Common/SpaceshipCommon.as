...
void Hover(CBlob@ this){
	...
	if(getMap().rayCastSolid(pos, pos + Vec2f(0, 32.0), groundPos)){
		f32 groundDistance = groundPos.y - pos.y;

		if(groundDistance < 30.0){
			if(this.isKeyPressed(key_up)){
				f32 forceAmount = this.getMass() * -0.4;
				this.AddForce(Vec2f(0, forceAmount));
			}
		}
		else if( groundDistance >= 30.0 && groundDistance <= 32){
...
void onTick(CBlob@ this){
	...
	if(occupied && this.hasTag("fullgravity")){
		Hover(this);
	}
}
